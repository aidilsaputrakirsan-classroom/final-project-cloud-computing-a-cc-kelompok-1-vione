@extends(
    ...
    (Auth::user()->roles == 'petugas' ? 'layout.petugas' : 'layout.hewan')
)
...
<div class="card shadow mb-4">
    <div class="card-header py-3">
        <h6 class="m-0 font-weight-bold text-primary">Tambah Data Hewan</h6>
    </div>
    <div class="card-body">
        <form action="{{ route('hewan.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            
            @if(in_array(Auth::user()->roles, ['admin', 'petugas']) && isset($hewanUsers) && count($hewanUsers) > 0)
            <div class="form-group">
                <label for="user_id">Pilih User Hewan</label>
                <select name="user_id" id="user_id" class="form-control @error('user_id') is-invalid @enderror">
                    <option value="">-- Pilih User --</option>
                    @foreach($hewanUsers as $hewanUser)
                        <option value="{{ $hewanUser->id }}">{{ $hewanUser->nama_user }} ({{ $hewanUser->username }})</option>
                    @endforeach
                </select>
                @error('user_id')
            ...
            <div class="form-group">
                ...
                <input type="text" name="nama_hewan" id="nama_hewan" class="form-control @error('nama_hewan') is-invalid @enderror" value="{{ old('nama_hewan', $user->nama_user) }}">
                @error('nama_hewan')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="text-center">
                ...
                @else
                    <a href="{{ route('dashboard-hewan') }}" class="btn btn-secondary">Kembali</a>
                @endif
            </div>
        </form>
    </div>
        ...
        if (userSelect) {
            userSelect.addEventListener('change', function() {
                if (this.value) {
                    ...
                    document.getElementById('nama_hewan').value = '{{ $user->nama_user }}';
