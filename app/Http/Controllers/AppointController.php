...
class AppointController extends Controller
{
    public function store(Request $request)
    {
        ...
        if ($user->role === 'shelter') {
            ...
            $pet = ShelterPet::find($request->shelter_pet_id);
        } else {
    ...
    {
        $shelterId = Auth::id();
        $appointments = Appointment::with(['vet', 'pet', 'medicalHistory'])
    ...
    public function ownerAppointments()
    {
        ...
        // Owner appointments with vet info
    ...
    {
        $appointment = Appointment::with(['pet', 'shelterPet'])->findOrFail($id);
        $user = auth()->user();
    ...
    public function vetAppointments()
    {
        ...
        // Vet appointments (with owner + pet)
    ...
    {
        $request->validate([
            // FIX: Tambahkan 'Completed' juga agar validasi lolos jika status ini digunakan
    ...
    public function updateVetFeedback(Request $request, $id)
    {
        ...
        // ✅ FIX: Set status menjadi 'Completed' setelah feedback diisi
