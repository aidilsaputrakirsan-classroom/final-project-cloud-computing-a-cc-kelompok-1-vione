...
class AntrianController extends Controller
{
    ...
    {
        $userId = Auth::id();
        $datahewan = Datahewan::where('user_id', $userId)->first();
        
        if ($datahewan) {
            $antrian = Antrian::with('jadwalpoliklinik')->where('id_hewan', $datahewan->id)->get();
    ...
    public function generatePDF()
    {
        ...
        if ($datahewan) {
            $antrian = Antrian::where('id_hewan', $datahewan->id)->get();
            $pdf = PDF::loadView('pdf/riwayat', ['antrian' => $antrian, 'datahewan' => $datahewan]);
            return $pdf->stream('Riwayat_Pendaftaran.pdf');
        }
        
        return back()->with('error', 'Tidak ada data hewan yang ditemukan');
    }
}